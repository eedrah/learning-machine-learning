...
projections = [
    np.outer(principal_component, eigenvector)
    for ((_, principal_component), eigenvector) in zip(
        principal_components.iteritems(), eigenvectors.T
    )
]


def main():
    """Display the PCA on a graph"""
